...
function updateProgressBar() {
  ...
  progressBar.innerHTML = steps
    .map(
      (_, i) => `<div class="progress-step ${i <= idx ? "active" : ""}"></div>`
    )
    .join("");
}
...
function blockEventIfActive(e) {
  if (
    ...
    !shouldLetEventThroughToSaver(e)
  ) {
    e.preventDefault();
...
[
  ...
  document.addEventListener(evt, (e) => blockEventIfActive(e), {
    capture: true,
    passive: false,
  });
});
  ...
  `;
    ...
    async function updateBrightness(value) {
      try {
        ...
        if (!data.success)
          console.warn("Brightness update failed:", data.error);
