...
`,
  ...
  finalize: (details) => `
        ...
        <table class="details-table">
            <tr><th><span class="material-icons">electric_meter</span>Meter ID</th><td>${
              details.meter_id
            }</td></tr>
            <tr><th><span class="material-icons">home</span>Household ID</th><td>${
              details.hhid || "Not set"
            }</td></tr>
            <tr><th><span class="material-icons">signal_cellular_alt</span>Connectivity</th><td>${
              details.connectivity
            }</td></tr>
            <tr><th><span class="material-icons">input</span>Input Sources</th><td>${
              details.input_sources.join(", ") || "None"
            }</td></tr>
            <tr><th><span class="material-icons">videocam</span>Video Detection</th><td>${
              details.video_detection ? "Working" : "Not working"
            }</td></tr>
        </table>
        <div class="button-group">
        ...
        <div class="layout-reset">
            <div class="main-dashboard fixed-layout">
                ...
                <div class="bottom-bar">
                    ...
                    <button class="bar-btn" onclick="showBrightnessPopup()"><span class="material-icons">brightness_medium</span><span>Brightness</span></button>
      ...
      (row, i) => `
        <div class="keyboard-row keyboard-row-${i}">
            ${row
              .map(
                (k) =>
                  `<button class="key" onclick="insertChar('${k}')">${k}</button>`
              )
              .join("")}
        </div>`
    )
...
/* ==============================================================
   BRIGHTNESS POPUP
   ============================================================== */
async function showBrightnessPopup() {
  const overlay = document.createElement("div");
  overlay.id = "bright-overlay";
  overlay.className = "overlay";

  const popup = document.createElement("div");
  popup.id = "bright-popup";
  popup.className = "popup";
  popup.innerHTML = `
        <h2><span class="material-icons">brightness_medium</span> Adjust Brightness</h2>
        <p>Slide to change screen brightness (5 levels)</p>
        <div id="bright-error" class="error" style="display:none;"></div>
        <div style="width:100%; padding:1rem 0; text-align:center;">
            <input type="range" id="brightness-slider" min="0" max="5" step="1" style="width:80%;">
            <div style="display:flex; justify-content:space-between; font-size:0.75rem; margin-top:0.5rem; color:hsl(var(--muted-foreground));">
                <span>Low</span><span>Medium</span><span>High</span>
            </div>
        </div>
        <div class="button-group">
            <button class="button secondary" onclick="closeBrightnessPopup()">Close</button>
        </div>`;
  ...
  try {
    const r = await fetch("/api/get_brightness");
    const d = await r.json();
    if (d.success) {
      const level = Math.round(d.value / 51);
      document.getElementById("brightness-slider").value = level;
    } else {
      showBrightError(d.error || "Failed to load brightness");
    }
  } catch {
    showBrightError("Network error");
  }
...
async function sendBrightness(val) {
  const err = document.getElementById("bright-error");
  try {
    const r = await fetch("/api/set_brightness", {
      method: "POST",
    ...
    const d = await r.json();
    if (!d.success) {
      showBrightError(d.error || "Failed to set brightness");
    } else if (err) {
...
function showBrightError(msg, type = "error") {
  const err = document.getElementById("bright-error");
  if (err) {
    err.className = type;
    err.innerHTML = `<span class="material-icons">${
...
function closeBrightnessPopup() {
  ["bright-popup", "bright-overlay"].forEach((id) => {
...
async function connectWiFi() {
  ...
  if (!ssid || !pass) {
    err.innerHTML =
      '<span class="material-icons">error</span> SSID & password required';
    err.className = "error";
    err.style.display = "flex";
    return;
  }
  try {
    ...
    }</span> ${d.success ? "Connected!" : d.error}`;
    err.style.display = "flex";
    if (d.success)
      setTimeout(async () => {
        closeWiFiPopup();
    ...
    err.innerHTML =
      '<span class="material-icons">error</span> Connection failed';
    err.style.display = "flex";
  }
}
...
async function disconnectWiFi() {
  ...
  try {
    ...
    err.innerHTML = `<span class="material-icons">${
      d.success ? "check_circle" : "error"
    }</span> ${d.message || d.error}`;
    err.style.display = "flex";
    if (d.success) setTimeout(scanWiFi, 2000);
  } catch {
    err.innerHTML =
      '<span class="material-icons">error</span> Disconnect failed';
    err.style.display = "flex";
  }
}
...
async function navigate(state, param = null) {
  ...
  if (state === "finalize") {
    const details = {
      ...
      video_detection:
        !!document.getElementById("video-status")?.dataset.detected,
    };
    render(details);
    return;
  }
...
async function fetchInputSources() {
  ...
  try {
    ...
    inputSources = [];
    ul.innerHTML =
      '<li><span class="material-icons">error</span> Detection failed</li>';
...
async function submitOTP() {
  ...
  if (btn) {
    btn.disabled = true;
    btn.innerHTML =
      '<span class="material-icons">hourglass_top</span> Verifying...';
  }
  try {
...
async function finalizeInstallation() {
  ...
  if (btn) {
    btn.disabled = true;
    btn.innerHTML =
      '<span class="material-icons">hourglass_top</span> Finalizing...';
  }
  try {
...
["click", "pointerdown", "touchstart", "pointermove", "mousemove"].forEach(
  (evt) => {
